...
func TestFsnotifyDirOnly(t *testing.T) {
	...
	{
		var f *os.File
		f, err := os.OpenFile(testFileAlreadyExists, os.O_WRONLY|os.O_CREATE, 0644)
		if err != nil {
			t.Fatalf("creating test file failed: %s", err)
		}
		f.Sync()
...
func TestFsnotifySubDir(t *testing.T) {
	...
	// Create a file
	var f *os.File
	f, err := os.OpenFile(testFile1, os.O_WRONLY|os.O_CREATE, 0644)
	if err != nil {
		t.Fatalf("creating test file failed: %s", err)
	}
	f.Sync()
	...
	var fs *os.File
	fs, err = os.OpenFile(testSubDirFile, os.O_WRONLY|os.O_CREATE, 0644)
	if err != nil {
		t.Fatalf("creating test file failed: %s", err)
	}
	fs.Sync()
...
func TestFsnotifyRenameToOverwrite(t *testing.T) {
	...
	// Create a file
	var fr *os.File
	fr, err := os.OpenFile(testFileRenamed, os.O_WRONLY|os.O_CREATE, 0644)
	if err != nil {
		t.Fatalf("creating test file failed: %s", err)
	}
	fr.Sync()
